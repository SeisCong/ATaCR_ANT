...
OUTPUTdir = [Workfolder 'NOISETC_CI/DATA/datacache_day_preproc/'];
...
samprate = 2; % new sample rate for each channel (note: for tilt/comp package must all be equal)
...
for ista = 1:length(stations)
    ...
    for ie =1 : length(data_filenames) % begin file loop for each station
        ...
        for ic = 1:length(channels) % begin channel loop
            ...
            %%%%%%%%%%%%%%%%%
            if resprm(ic) ==1
                if (isempty(traces_day(1).sacpz.poles) |...
                    isempty(traces_day(2).sacpz.poles) |...
                    isempty(traces_day(3).sacpz.poles) |...
                    isempty(traces_day(4).sacpz.poles)) &...
                   isempty(pole_zero_dir)
                    prob = 1;
                    continue
                end
