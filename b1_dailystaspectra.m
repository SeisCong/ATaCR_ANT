...
%% Adding output for daily waveforms
%% Modified by Cong Li

clear; close all

% CODE OPTIONS
isfigure_waveform = 1; % generate waveform for each day by L.C.
...
for ista = 7:length(stations)
    ...
    for ie = 1 : length(day_filenames)
        ...
        if isfigure_waveform ==1 %% Add by L.C.
        figure(66);clf
        end
        ...
        % Draw daily waveforms; Added by L.C.
        if isfigure_waveform && issavefigure
        figure(66)
        set(gcf,'PaperPositionMode','manual');
        set(gcf,'PaperUnits','inches');
        set(gcf,'PaperOrientation','portrait');
        set(gcf,'PaperPosition',[.05 .05 8 10.5]);
        subplot(4,1,1)
           plot(taxisZ,Zraw,'color',[1,0,0]);
           ylabel('mm');
           title('Z');
           for wi=1:length(goodwins)
               hold on; plot(taxisZ(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts)...
                   ,Zraw(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts),'color',[0,0,0],'linewidth',1.1);
           end
        subplot(4,1,2);
           plot(taxis1,H1raw,'color',[1,0,0]);
           for wi=1:length(goodwins)
                              hold on; plot(taxisZ(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts)...
                   ,H1raw(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts),'color',[0,0,0],'linewidth',1.1);
           end
           ylabel('mm');
           title('H1');
        subplot(4,1,3);
           plot(taxis2,H2raw,'color',[1,0,0]);
           for wi=1:length(goodwins)
                             hold on; plot(taxisZ(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts)...
                   ,H2raw(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts),'color',[0,0,0],'linewidth',1.1);
           end
           ylabel('mm');
           title('H2')
        subplot(4,1,4);
           plot(taxisP,Praw,'color',[1,0,0]);
           for wi=1:length(goodwins)
                             hold on; plot(taxisZ(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts)...
                   ,Praw(iptwin1(goodwins(wi))+(T/2)/dt:iptwin2(goodwins(wi))+(T/2)/dt-overlap*npts),'color',[0,0,0],'linewidth',1.1);
           end
           ylabel('pascal');
           xlabel('time (s)');
           title('P');
        filename=sprintf('%s/%s_%s_waveform',figoutpath,netsta,dayid);
        print(gcf,'-dpng',filename)
        end
