function [Zraw,H1raw,H2raw,Praw,taxisZ,taxis1,taxis2,taxisP,dt] = varsetup_correctdailyrecord(sta,chz_vec,ch1_vec,ch2_vec,chp_vec,T)
%% Cong Li
%% conli@geo.umass.edu
idxZ = find(ismember({sta.traces_day.channel},chz_vec));
idx1 = find(ismember({sta.traces_day.channel},ch1_vec));
idx2 = find(ismember({sta.traces_day.channel},ch2_vec));
idxP = find(ismember({sta.traces_day.channel},chp_vec));

Z = sta.traces_day(idxZ);
H1 = sta.traces_day(idx1);
H2 = sta.traces_day(idx2);
P = sta.traces_day(idxP);
rateZ = Z.sampleRate;
rate1 = H1.sampleRate;
rate2 = H2.sampleRate;
rateP = P.sampleRate;
