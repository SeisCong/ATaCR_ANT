function trans_filename=correctdaily_findTFs(inpath_trans,tf_op,station,eventid);
...
if ~isempty(trans_mat_files)
    ...
    elseif tf_op ==1
        ...
        iday = find(dday == 0); % find days 
%         [~,imin] = min(dday(iday)); % find min index
%         min_idx = iday(imin); % index day nearest to event
        if isempty(iday) % if no available day prior, use nearest day
