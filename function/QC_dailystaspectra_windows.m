...
%     sla_D(:,ii) = la_D(:,ii);
    ...
    dsla_D(:,:,ic) = sla_D-mean(sla_D,2)*ones(1,Nwin); % spectrum pertubation for each window
    ...
    for ic = 1:4
        if comp_exist(ic)==1
            for ii = 1:length(goodwins)
                ind = goodwins; ind(ii) = [];
                normvar(ii) = norm(std(dsla_D(:,ind,ic),0,2)); % the norm of std for spectrum pertubation of all windows in a certain channel
            end
            ubernorm(:,in) = [normvar];
            in = in+1;
        end
